...
static void fiq_debugger_dump_kernel_log(struct fiq_debugger_state *state)
{
	...
	struct kmsg_dump_iter dumper_iter;

	//kmsg_dump_rewind_nolock(&dumper);
	kmsg_dump_rewind(&dumper_iter);
	//while (kmsg_dump_get_line_nolock(&dumper, true, buf,
	//				 sizeof(buf) - 1, &len)) {
	while (kmsg_dump_get_line(&dumper_iter, true, buf, 
					 sizeof(buf) - 1, &len)) {
		buf[len] = 0;
...
ssize_t  fiq_tty_write(struct tty_struct *tty, const unsigned char *buf, size_t count)
{
	int i;
	int line = tty->index;
	struct fiq_debugger_state **states = tty->driver->driver_state;
...
static int fiq_debugger_tty_init(void)
{
	...
	fiq_tty_driver = tty_alloc_driver(MAX_FIQ_DEBUGGER_PORTS, 0);
	if (!fiq_tty_driver) {
